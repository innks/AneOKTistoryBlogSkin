  ...
  $.fn.DynamicScrollspy = function(opts) {
    ...
    function makeTree() {
      ...
      $('H' + self.options.tH).not(self.options.exclude).each(function() {
        ...
        var n = $(this).html();
        var s;
        if (n.includes("&")) s = n.substr(0, n.indexOf("&"));
        else if (n.includes("<")) {
          if (n.includes("<em>")) {n = n.replace("<em>", "");n = n.replace("</em>", "");s = n;} else s = n.substr(0, n.indexOf("<"));
        } else s = n;
        // } 추가 
        ...
        s = s.trim();
        tree[$(this).prop('id')] = {
          dstext: encodeHTML(s), // dstext: encodeHTML($(this).text()), -> dstext: encodeHTML(s), 로 변경
          jqel: $(this)
        };
      });
    ...
    function itCreateTree(what) {
      for (var k in what) {
        ...
        $('#' + k).nextUntil('H' + lvl).filter('H' + (lvl + 1)).not(self.options.exclude).each(function() {
          ...
          var i = $(this).html();
          var n;
          if (i.includes("&")) n = i.substr(0, i.indexOf("&"));
          else if (i.includes("<")) {
            if (i.includes("<em>")) {n = i.replace("<em>", "");n = i.replace("</em>", "");n = i;} else n = i.substr(0, i.indexOf("<")); 
          } else var n = i;
          n = n.trim();
          // } 추가 

          what[k][$(this).prop('id')] = {
            dstext: encodeHTML(n), // dstext: encodeHTML($(this).text()), -> dstext: encodeHTML(n), 로 변경
            jqel: $(this)
          };
        });
