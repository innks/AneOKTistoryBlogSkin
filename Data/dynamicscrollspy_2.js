...
*
*  adding v 0.0.2 by AneOK blog IN:nK
*  https://aneok.tistory.com/
*/
...
(function($) {
  ...
  $.fn.DynamicScrollspy = function(opts) {
    ...
    this.options = $.extend({}, {
      ...
      hMark: true //show heading '#' / 제목 뒤에 '#' 붙이기
    }, this.options, opts);
    ...
    function showHMark() {
      selectAllH().append(function() {
        return ' <a class="ak-h-' + $(this).prop('tagName') + '" href="#' + $(this).prop('id') +'">#</a>';
      });
    }    
    ...
    function makeTree() {
      ...
      $('H' + self.options.tH).not(self.options.exclude).each(function() {
        ...
        if (self.options.hMark) n.substr(0, n.length);
    ...
    function itCreateTree(what) {
        ...
        $('#' + k).nextUntil('H' + lvl).filter('H' + (lvl + 1)).not(self.options.exclude).each(function() {
          ...
          if (self.options.hMark) i.substr(0, i.length);
    ...
    function renderTree() {
      ...
      $.each(self.tree, function(k) {
        var c = self.tree[k];
        var li = '<li id="AoK-' + k + '" class="nav-item"><a href="#' + k + '" onclick="tocHGo(' + k + '); return false;" class="nav-link">' + c.dstext + '</a></li>';
        ul.append(li);
    ...
    function itRenderTree(what) {
      ...
      for (var k in what) {
        ...
        if (k == 'dstext' || k == 'jqel') continue;
        var c = what[k];        
        ul.append('<li id="AoK-' + k + '" class="nav-item"><a href="#' + k + '" onclick="tocHGo(' + k + '); return false;" class="nav-link">' + c.dstext + '</a></li>');
        itRenderTree(what[k]);
      }
    }
    ...
    function init() {
      ...
      if (self.isinit === false) {
        ...
        if (self.options.hMark) showHMark();
